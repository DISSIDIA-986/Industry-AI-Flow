...
def test_chinese_visualization():
    ...
    bars = plt.bar(furnish_avg.index, furnish_avg.values, color=colors_furnish, edgecolor='black')
    ...
    bars = plt.bar(mainroad_avg.index, mainroad_avg.values, color=colors_mainroad, edgecolor='black')
    ...
    bars = plt.bar(parking_avg.index, parking_avg.values, color='lightgreen', edgecolor='black')
    ...
    plt.axvline(df['price'].mean(), color='red', linestyle='--', linewidth=2, label=f'平均值: {df["price"].mean():,.0f}')
    plt.xlabel('房价（卢比）')
    plt.ylabel('频次')
    ...
    plt.axvline(df['area'].mean(), color='red', linestyle='--', linewidth=2, label=f'平均值: {df["area"].mean():,.0f}')
    plt.xlabel('面积（平方英尺）')
    plt.ylabel('频次')
    ...
    furnish_labels_map = {'furnished': '精装修', 'semi-furnished': '半装修', 'unfurnished': '毛坯房'}
    labels = [furnish_labels_map.get(x, x) for x in furnish_counts.index]
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1']
    explode = (0.05, 0.05, 0.05)
    plt.pie(furnish_counts.values, labels=labels, autopct='%1.1f%%', colors=colors, explode=explode)
    ...
    # 下方左2：卧室数vs价格
    plt.subplot(2, 4, 6)
    bedroom_data = [df[df['bedrooms'] == i]['price'].values for i in sorted(df['bedrooms'].unique())]
...
"""
    plt.text(0.05, 0.95, insights_text, transform=plt.gca().transAxes, fontsize=9,
...
def test_chinese_text_rendering():
    ...
    plt.subplot(2, 2, 4)
    # 测试复杂的中文文本
    plt.text(0.5, 0.8, '中文字体测试', ha='center', va='center',
             fontsize=16, fontweight='bold', transform=plt.gca().transAxes)
    plt.text(0.5, 0.6, '支持中文显示！', ha='center', va='center',
             fontsize=14, color='red', transform=plt.gca().transAxes)
    plt.text(0.5, 0.4, '包含标点符号：，。！？', ha='center', va='center',
             fontsize=12, color='blue', transform=plt.gca().transAxes)
    plt.text(0.5, 0.2, '数字和单位：123.45万元', ha='center', va='center',
