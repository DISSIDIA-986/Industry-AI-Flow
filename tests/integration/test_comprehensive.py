...
def log_test(result: TestResult):
    ...
    print(
        f"{status_symbol} {result.test_id}: {result.status} ({result.execution_time:.3f}s)"
    )
    if result.notes:
    ...
    try:
        ...
        log_test(
            TestResult(
                test_id,
                "OCR",
                "PASS" if version_check else "FAIL",
                time.time() - start_time,
    ...
    summary = {
        ...
        "quality_metrics": {
            ...
            "workflow_tests": sum(
                ...
                if "Workflow" in r.category or "API" in r.category
            ),
            "environment_tests": sum(
                1 for r in test_results if "Environment" in r.category
            ),
        },
