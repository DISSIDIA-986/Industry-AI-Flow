        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "keyword_intent_mapping":
...
class TestR12_02_ClarificationTemplates:
    ...
    def test_templates_contain_real_questions(self):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "clarification_templates":
                        assert isinstance(node.value, ast.Dict)
                        ...
                        for q in all_questions:
                            words = [w for w in q.split() if len(w) >= 3 and w.upper() != "EN"]
                            assert len(words) >= 3, \
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_generate_clarification_with_prompt":
                    ...
                    for s in visible:
                        ...
                        assert en_words / total_words < 0.3, \
                    ...
                    real_words = [w for w in all_text.split() if len(w) >= 4 and w.upper() != "EN"]
                ...
                if node.name == "_build_code_generation_prompt":
                    ...
                    for s in fn_strings:
                        if len(s.strip()) > 10:
                            ...
                            assert en_count / word_count < 0.3, \
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_template_count":
                    ...
                    assert "'EN:" not in fn_source and '"EN:' not in fn_source, \
                        f"_template_count still has EN labels"
                    # Should contain real English labels
                    assert "count" in fn_source.lower() or "total" in fn_source.lower(), \
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_template_percentage":
                    ...
                    assert "percentage" in fn_source.lower() or "distribution" in fn_source.lower(), \
                    ...
                    for child in ast.walk(node):
                        if isinstance(child, ast.List):
                            for elt in child.elts:
                                if isinstance(elt, ast.Constant) and isinstance(elt.value, str):
                                    assert elt.value.strip().upper() != "EN", \
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_generate_template_code":
                    ...
                    assert "avg" in all_keywords or "average" in all_keywords, \
                        "Missing average-related keywords"
                    assert "maximum" in all_keywords or "largest" in all_keywords, \
                        "Missing max-related keywords"
                    assert "minimum" in all_keywords or "smallest" in all_keywords, \
                        "Missing min-related keywords"
                    assert "total" in all_keywords or "number of" in all_keywords, \
                        "Missing count-related keywords"
                    assert "proportion" in all_keywords or "ratio" in all_keywords, \
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_parse_execution_output":
                    ...
                    for s in visible:
                        en_tokens = sum(1 for w in s.split() if w.strip().upper() == "EN")
                        total_words = max(len(s.split()), 1)
                        assert en_tokens / total_words < 0.3, \
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if node.name == "_generate_fallback_answer":
                    ...
                    assert "dataset" in all_text or "column" in all_text or "analysis" in all_text, \
