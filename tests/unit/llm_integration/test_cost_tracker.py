...
from backend.services.llm_integration.cost_tracker import CostTracker, LLMCost, LLMUsage
...
class TestCostTracker:
    ...
    def test_record_usage_basic(self, tracker):
        ...
        tracker.record_usage(
            tenant_id="tenant1", provider="zhipu", model="glm-4", usage=usage
        )
    ...
    def test_concurrent_record_usage(self):
        ...
        # EN100EN
        threads = [
            threading.Thread(target=record_concurrently, args=(i,)) for i in range(100)
        ]
    ...
    def test_usage_total_tokens_calculation(self, tracker):
        ...
        # ENtotal_tokens,EN
        usage2 = LLMUsage(prompt_tokens=100, completion_tokens=50, total_tokens=200)
    ...
    @pytest.mark.parametrize(
        "tenant_id,provider,model,prompt_tokens,completion_tokens",
        [
    ...
    def test_record_usage_parametrized(
        self, tracker, tenant_id, provider, model, prompt_tokens, completion_tokens
    ):
        """EN"""
        usage = LLMUsage(
        ...
        tracker.record_usage(
            tenant_id=tenant_id, provider=provider, model=model, usage=usage
        )
