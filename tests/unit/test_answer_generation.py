...
class AnswerGenerationTester:
    ...
    def _generate_mock_answer(self, test_case: TestCase) -> str:
        ...
        return answer_templates.get(test_case.test_id, f"这是一个关于{test_case.topic}的模拟回答，基于提供的上下文生成。")
    ...
    def _check_factual_accuracy(self, answer: str, test_case: TestCase) -> float:
        ...
        has_errors = any(re.search(pattern, answer, re.IGNORECASE) for pattern in error_patterns)
        if has_errors:
            return 0.3
    ...
    def _check_logical_consistency(self, answer: str, test_case: TestCase) -> float:
        ...
        has_contradictions = any(re.search(pattern, answer, re.IGNORECASE) for pattern in contradiction_patterns)
    ...
    def _check_content_completeness(self, answer: str, test_case: TestCase) -> float:
        ...
        # 检查是否涵盖关键方面
        key_aspects_score = self._check_keyword_coverage(answer, test_case.expected_keywords)
    ...
    def _check_context_consistency(self, answer: str, context: str) -> float:
        ...
        has_contradiction = any(indicator in answer for indicator in contradiction_indicators)
    ...
    def test_answer_generation_quality(self) -> Dict[str, Any]:
        ...
        for test_case in self.test_cases:
            ...
            completeness_score = self._check_content_completeness(generated_answer, test_case)

            # 计算总分
            overall_score = (correctness_score * 0.3 +
                           fluency_score * 0.2 +
                           relevance_score * 0.3 +
            ...
            detailed_metrics = {
                ...
                "keyword_coverage": self._check_keyword_coverage(generated_answer, test_case.expected_keywords),
            ...
            print(f"    ✅ 总分: {overall_score:.3f} (正确性:{correctness_score:.3f}, 流畅性:{fluency_score:.3f}, 相关性:{relevance_score:.3f})")
    ...
    def analyze_performance_by_answer_type(self, results: List[AnswerQuality]) -> Dict[str, Any]:
        """按回答类型分析性能"""
        ...
        for answer_type, type_results in type_groups.items():
            ...
            avg_scores = {
                "correctness": sum(r.correctness_score for r in type_results) / len(type_results),
                "fluency": sum(r.fluency_score for r in type_results) / len(type_results),
                "relevance": sum(r.relevance_score for r in type_results) / len(type_results),
                "completeness": sum(r.completeness_score for r in type_results) / len(type_results),
                "overall": sum(r.overall_score for r in type_results) / len(type_results)
            }
    ...
    def test_generation_speed(self) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(self.test_cases[:5]):  # 测试前5个用例
            ...
            print(f"  ⏱️  用例 {i+1}: {generation_time:.4f}s ({len(generated_answer)} 字符)")
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        # 2. 按类型分析性能
        type_analysis = self.analyze_performance_by_answer_type(quality_results["results"])
        ...
        # 计算总体指标
        overall_correctness = sum(r.correctness_score for r in quality_results["results"]) / len(quality_results["results"])
        overall_fluency = sum(r.fluency_score for r in quality_results["results"]) / len(quality_results["results"])
        overall_relevance = sum(r.relevance_score for r in quality_results["results"]) / len(quality_results["results"])
        overall_quality = sum(r.overall_score for r in quality_results["results"]) / len(quality_results["results"])
...
def main():
    ...
    # 保存JSON结果
    with open(f"test_results/answer_generation_results_{timestamp}.json", "w", encoding="utf-8") as f:
    ...
    # 保存报告
    with open(f"test_results/answer_generation_report_{timestamp}.md", "w", encoding="utf-8") as f:
        f.write(report)
