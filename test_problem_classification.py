...
class ProblemClassificationTester:
    ...
    def setup(self):
        ...
        try:
            from backend.services.intent_classification.intent_classifier import IntentClassifier, QueryContext
    ...
    async def classify_with_validation_async(self, query: str, expected_intent: str,
    ...
    def classify_with_validation(self, query: str, expected_intent: str,
                                min_confidence: float = 0.7) -> Dict:
        """
        ...
        Returns:
            Dict with test result
        """
        return asyncio.run(self.classify_with_validation_async(query, expected_intent, min_confidence))
    ...
    def test_1_1_knowledge_retrieval(self):
        ...
        for query, expected in test_cases:
            ...
            if result["passed"]:
                self.stats["passed"] += 1
                logger.info(f"✅ PASS: {query[:30]}... → {result['actual_intent']} ({result['confidence']:.2f})")
            else:
                self.stats["failed"] += 1
                logger.error(f"❌ FAIL: {query[:30]}... → Expected: {expected}, Got: {result['actual_intent']}")
    ...
    def test_2_1_mixed_intent(self):
        ...
        results = []
        for query, expected in test_cases:
            result = self.classify_with_validation(query, expected, min_confidence=0.6)  # Lower threshold for complex
            results.append(result)
            self.stats["total_tests"] += 1
            if result["passed"]:
                self.stats["passed"] += 1
                logger.info(f"✅ PASS: {query[:40]}... → {result['actual_intent']} ({result['confidence']:.2f})")
            else:
                self.stats["failed"] += 1
                logger.warning(f"⚠️ PARTIAL: {query[:40]}... → Expected: {expected}, Got: {result['actual_intent']}")
    ...
    def test_2_2_ambiguous_queries(self):
        ...
        for query, acceptable_intents in test_cases:
            result = self.classify_with_validation(query, acceptable_intents[0], min_confidence=0.5)
            # Override pass condition to accept any of the acceptable intents
            result["passed"] = result["actual_intent"] in acceptable_intents if result["actual_intent"] else False
            results.append(result)
            self.stats["total_tests"] += 1
            if result["passed"]:
                self.stats["passed"] += 1
                ...
                self.stats["failed"] += 1
                logger.warning(f"⚠️ AMBIGUOUS: {query} → {result['actual_intent']}, expected one of {acceptable_intents}")
    ...
    def test_4_1_edge_cases(self):
        ...
        for query, expected in test_cases:
            try:
                result = self.classify_with_validation(query, expected, min_confidence=0.0)
        ...
        for query in test_queries:
            ...
            try:
                ...
                if response_time < 0.5:
                    ...
                    logger.warning(f"⚠️ SLOW: {query[:30]}... → {response_time*1000:.1f}ms")
            except Exception as e:
    ...
    async def test_6_1_error_recovery_async(self):
        ...
        for invalid_input, description in error_cases:
            try:
                ...
                result = await self.classifier.classify_intent(invalid_input, self.default_context)
                ...
                # Unexpected error
                logger.warning(f"⚠️ PARTIAL: {description} raised {type(e).__name__}: {e}")
    ...
    def save_results(self, output_file: str = "test_results/problem_classification_results.json"):
