...
class ComprehensiveDatabaseInitializer:
    ...
    def check_extension(self, extension_name: str) -> bool:
        """检查扩展是否可用"""
        try:
            self.cursor.execute(f"SELECT 1 FROM pg_extension WHERE extname = '{extension_name}'")
...
Response format: JSON with "intent" and "confidence" fields.''',
                    ...
                    'metadata': {'description': 'Intent classification for routing decisions', 'author': 'system'},
                    'priority': 100,
                    'is_active': True,
                    'is_latest': True
                },
...
5. Include relevant source citations if applicable''',
                    'variables': {'context': 'retrieved context', 'question': 'user question'},
                    'metadata': {'description': 'RAG response generation with retrieved context', 'author': 'system'},
                    'priority': 95,
                    'is_active': True,
                    'is_latest': True
                },
...
Provide your analysis in a structured format with specific code examples where applicable.''',
                    ...
                    'metadata': {'description': 'Comprehensive code analysis assistant', 'author': 'system'},
                    'priority': 90,
                    'is_active': True,
                    'is_latest': True
                },
...
Format the output as structured JSON with clear categorization.''',
                    ...
                    'metadata': {'description': 'Building document OCR processing and structuring', 'author': 'system'},
                    'priority': 85,
                    'is_active': True,
                    'is_latest': True
                }
    ...
    def verify_schema(self) -> Dict[str, any]:
        ...
        try:
            ...
            # 检查表
            expected_tables = [
                'documents', 'document_chunks', 'query_feedback', 'document_quality_scores',
                'query_optimization_log', 'document_versions', 'document_operations_log',
                'prompts', 'prompt_versions', 'prompt_usage_logs', 'prompt_experiments',
                'prompt_tags', 'prompt_tag_relations', 'prompt_daily_stats',
                'prompt_weekly_stats', 'prompt_monthly_stats'
            ]
            ...
            verification_result['views']['prompt_summary'] = 'prompt_summary' in existing_views

            # 统计结果
            total_items = len(expected_tables) + len(expected_extensions) + 1
            passed_items = sum(
                verification_result['tables'][table] for table in expected_tables
            ) + sum(
                verification_result['extensions'][ext] for ext in expected_extensions
    ...
    def initialize_database(self) -> bool:
        ...
        try:
            ...
            # 执行SQL架构文件
            schema_file = Path(__file__).parent / "migrations" / "001_create_comprehensive_schema.sql"
            if schema_file.exists():
                logger.info("Executing comprehensive schema from SQL file...")
                if not self.execute_sql_file(str(schema_file)):
                    return False
            else:
                logger.warning("SQL schema file not found, using manual initialization...")
                # 这里可以添加手动初始化逻辑
            ...
            logger.info(f"Success rate: {verification_result.get('success_rate', 0):.1f}%")
