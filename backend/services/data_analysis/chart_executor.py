...
def _build_combined_snippet(
    ...
    lines: List[str] = [
        ...
        _loader_block(ext),
        "",
        # Non-finite hardening: matplotlib cannot plot +/-inf (raises
        # "supplied range is not finite"), and json.dumps emits the literal
        # tokens NaN/Infinity for non-finite floats — valid for Python's
        # json.loads but INVALID JSON that breaks a browser JSON.parse. We
        # strip inf from series before plotting and sanitize every emitted
        # summary so the marker line is always spec-valid JSON.
        "import math as _math",
        "def _finite_series(s):",
        "    return s.replace([_np.inf, -_np.inf], _np.nan).dropna()",
        "",
        "def _san(o):",
        ...
        "    print(f'{marker}=' + _json.dumps(_san(payload), default=str, allow_nan=False))",
        "",
...
def _render_histogram(params: Dict[str, Any], image_filename: str) -> List[str]:
    ...
    return [
        ...
        "_series = _finite_series(_pd.to_numeric(_df[_col], errors='coerce'))",
        "if _series.empty:",
        "    raise ValueError(f'column {_col!r} has no finite numeric values')",
...
def _render_scatter(params: Dict[str, Any], image_filename: str) -> List[str]:
    ...
    return [
        ...
        "_x = _pd.to_numeric(_df[_x_col], errors='coerce').replace([_np.inf, -_np.inf], _np.nan)",
        "_y = _pd.to_numeric(_df[_y_col], errors='coerce').replace([_np.inf, -_np.inf], _np.nan)",
...
def _render_heatmap(params: Dict[str, Any], image_filename: str) -> List[str]:
    ...
    return [
        ...
        "_num = _pd.DataFrame({_c: _pd.to_numeric(_df[_c], errors='coerce').replace([_np.inf, -_np.inf], _np.nan) for _c in _cols})",
...
def _render_boxplot(params: Dict[str, Any], image_filename: str) -> List[str]:
    ...
    return [
        ...
        "_plot[_col] = _pd.to_numeric(_plot[_col], errors='coerce').replace([_np.inf, -_np.inf], _np.nan)",
...
def _build_model_snippet(mc: Dict[str, Any], data_file_path: str) -> str:
    ...
    lines: List[str] = [
        ...
        "",
        # Same non-finite guard as the EDA emitter: a degenerate metric (e.g. R^2
        # on a constant target, or an overflowed score) can be NaN/inf, which
        # json.dumps would serialize as the invalid JSON tokens NaN/Infinity and
        # break the browser's JSON.parse. Sanitize to null and forbid nan tokens.
        "import math as _math",
        "def _san(o):",
        "    if isinstance(o, bool):",
        "        return o",
