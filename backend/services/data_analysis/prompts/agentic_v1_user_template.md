...
- **Non-finite values (mandatory):** real-world numeric columns can contain `inf`/`-inf` (e.g. a ratio with a zero denominator) or magnitudes that overflow. matplotlib raises `"supplied range is not finite"` when asked to plot them, and non-finite stats serialize as the invalid JSON tokens `NaN`/`Infinity`. Before plotting OR computing summary statistics on a numeric column, strip non-finite values (numpy is already imported as `np` per the reproducibility rule above): `col = pd.to_numeric(df[name], errors="coerce").replace([np.inf, -np.inf], np.nan).dropna()`. Never put a raw `inf`/`nan` float into the `summary` dict.
