...
请只返回Python代码，不要包含任何解释。将代码放在```python 和 ```之间。"""
    ...
    def _generate_template_code(
        ...
        elif any(keyword in question_lower for keyword in ['max', '最高', '最大', 'highest']):
            return self._template_max(filename, dataset_metadata)

        elif any(keyword in question_lower for keyword in ['min', '最低', '最小', 'lowest']):
            return self._template_min(filename, dataset_metadata)

        elif any(keyword in question_lower for keyword in ['count', '数量', '个数', 'how many']):
            return self._template_count(filename, dataset_metadata)

        elif any(keyword in question_lower for keyword in ['percentage', '百分比', '比例', '%']):
...
"""
    ...
    def _template_average(self, filename: str, metadata: Dict) -> str:
        """平均值计算模板"""
        numeric_cols = [
            col['name'] for col in metadata.get('columns_info', [])
            if 'mean' in col
        ]
...
"""
    ...
    def _generate_fallback_answer(
        ...
        # 列名查询
        if 'feature' in question_lower or 'column' in question_lower or '特征' in question_lower or '列' in question_lower:
            columns = dataset_metadata.get('column_names', [])
            if columns:
                return f"数据集包含以下列: {', '.join(columns)}"

        # 行数查询
        if 'how many' in question_lower or '多少' in question_lower or 'row' in question_lower or '行' in question_lower:
