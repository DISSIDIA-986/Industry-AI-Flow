...
_EMIT_SUMMARY_HELPER = (
    ...
    "            except Exception:\n"
    "                return str(o)\n"
    # Non-finite guard: scipy/sklearn/pandas can yield inf/nan (R^2 on a constant\n"
    # target, a ratio with zero denominator). json.dumps would serialize those as\n"
    # the invalid JSON tokens Infinity/NaN and break the browser's JSON.parse.\n"
    # numpy.float64 is a subclass of float, so this catches it too; anything that\n"
    # still slips is caught by allow_nan=False -> the valid-JSON fallback below.\n"
    "    def _san(o):\n"
    "        if isinstance(o, bool):\n"
    "            return o\n"
    "        if isinstance(o, float):\n"
    "            return o if _ds_math.isfinite(o) else None\n"
    "        if isinstance(o, dict):\n"
    "            return {_k: _san(_v) for _k, _v in o.items()}\n"
    "        if isinstance(o, (list, tuple)):\n"
    "            return [_san(_v) for _v in o]\n"
    "        return o\n"
    "    try:\n"
    "        _payload = _ds_json.dumps(_san(_summary), default=_coerce, allow_nan=False)\n"
