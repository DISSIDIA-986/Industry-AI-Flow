...
class OllamaClient:
    def __init__(self, base_url: str = None, model: str = None, default_temperature: float = 0.7,
                 default_max_tokens: int = 2000, default_top_p: float = 0.9):
    ...
    def _handle_stream_response(self, response) -> str:
        ...
        for line in response.iter_lines():
            if line:
                try:
                    data = response.json() if hasattr(response, 'json') else eval(line.decode())
                    if "response" in data:
    ...
    def chat(self, messages: list, temperature: Optional[float] = None,
             max_tokens: Optional[int] = None, top_p: Optional[float] = None) -> str:
        """
        对话模式生成
