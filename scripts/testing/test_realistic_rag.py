...
class RealisticRAGTester:
    ...
    def test_question(self, dataset: str, q_data: Dict) -> Dict:
        ...
        try:
            ...
            keywords_found = [kw for kw in q_data["keywords"] if kw.lower() in answer]
            keyword_match_rate = len(keywords_found) / len(q_data["keywords"])

            print(f"\n🔍 相关性检查:")
            print(
                f"   关键词匹配: {len(keywords_found)}/{len(q_data['keywords'])} ({keyword_match_rate*100:.1f}%)"
            )
            if keywords_found:
    ...
    def generate_report(self):
        ...
        if successful > 0:
            ...
            if intent_results:
                intent_correct = sum(
                    1 for r in intent_results if r.get("intent_correct")
                )
                intent_accuracy = intent_correct / len(intent_results) * 100
                avg_confidence = sum(
                    r.get("intent_confidence", 0) for r in intent_results
                ) / len(intent_results)

                print(f"\n🎯 意图分类准确率:")
                print(
                    f"   正确: {intent_correct}/{len(intent_results)} ({intent_accuracy:.1f}%)"
                )
            ...
            for difficulty in ["简单", "中等", "困难"]:
                ...
                if diff_results:
                    avg_time = sum(r.get("rag_time", 0) for r in diff_results) / len(
                        diff_results
                    )
                    avg_match = sum(
                        r.get("keyword_match_rate", 0) for r in diff_results
                    ) / len(diff_results)
                    print(
                        f"   {difficulty}: {len(diff_results)}题, "
            ...
            for dataset_name in TEST_QUESTIONS.keys():
                ...
                if ds_results:
                    avg_match = sum(
                        r.get("keyword_match_rate", 0) for r in ds_results
                    ) / len(ds_results)
                    avg_sources = sum(
                        r.get("sources_count", 0) for r in ds_results
                    ) / len(ds_results)
                    print(
                        f"   {dataset_name}: {len(ds_results)}题, "
