...
class VersionManager:
    ...
    def check_python_version(self) -> Tuple[bool, str]:
        ...
        if current_version == target_version:
            return (
                True,
                f"✅ Python版本完美匹配: {current_version[0]}.{current_version[1]} (PaddleOCR核心版本)",
            )
        else:
            return (
                False,
                f"❌ Python版本不匹配: {current_version[0]}.{current_version[1]} ≠ {target_version[0]}.{target_version[1]} (需要Python 3.13以支持PaddleOCR)",
            )
    ...
    def check_dependency_compatibility(self, dep_name: str) -> Tuple[bool, str]:
        ...
        if current_py_version != target_version:
            return (
                False,
                f"❌ {dep_name} 需要Python {target_version[0]}.{target_version[1]} (当前: {current_py_version[0]}.{current_py_version[1]})",
            )

        # 检查核心依赖
        if dep_name in self.version_requirements["python"]["critical_dependencies"]:
            dep_info = self.version_requirements["python"]["critical_dependencies"][
                dep_name
            ]
            return (
                True,
                f"✅ {dep_name} 是核心依赖，Python 3.13完全支持 - {dep_info.get('notes', '')}",
            )

        # 检查可选依赖
        if dep_name in self.version_requirements["python"]["optional_dependencies"]:
            dep_info = self.version_requirements["python"]["optional_dependencies"][
                dep_name
            ]
            return (
                True,
                f"✅ {dep_name} 是可选依赖，Python 3.13支持 - {dep_info.get('notes', '')}",
            )
        ...
        if current_py_version in self.compatibility_matrix:
            ...
            if dep_info.get("supported", False):
                return (
                    True,
                    f"✅ {dep_name} 在Python 3.13下完全支持 - {dep_info.get('notes', '')}",
                )
            else:
                return (
                    False,
                    f"❌ {dep_name} 在Python 3.13下不支持 - {dep_info.get('notes', '')}",
                )
        else:
    ...
    def generate_version_report(self) -> Dict:
        ...
        for package, installed, msg in package_results:
            report["package_status"][package] = {"installed": installed, "message": msg}
        ...
        if not venv_ok:
            report["recommendations"].append(
                "🔒 建议创建虚拟环境：python3.13 -m venv venv && source venv/bin/activate"
            )

        # 根据缺失的包给出安装建议
        missing_packages = [
            pkg for pkg, installed, _ in package_results if not installed
        ]
        if missing_packages:
            if current_version == target_version:
                # Python 3.13 专用安装建议
                report["recommendations"].append("🔧 Python 3.13环境安装命令：")
                report["recommendations"].append(
    ...
    def print_version_report(self):
        ...
        for component, status in report["compatibility"].items():
            ...
            component_name = {"python": "Python版本", "virtual_environment": "虚拟环境"}.get(
                component, component
            )
    ...
    def save_report(self, filename: str = None):
        ...
        if filename is None:
            ...
            filename = (
                f"version_compatibility_report_{time.strftime('%Y%m%d_%H%M%S')}.json"
            )
