...
class EnhancedPGVector(PGVector):
    ...
    def similarity_search_with_filter(
        ...
        if doc_ids:
            ...
            where_clauses.append(f"dc.doc_id IN ({placeholders})")
        ...
        # 完整SQL
        full_sql = f"{base_sql} WHERE {' AND '.join(where_clauses)} ORDER BY distance LIMIT %s"
        params.append(top_k)
...
class EnhancedHybridRetriever(BaseRetriever):
    ...
    def _bm25_search(self, query: str, doc_ids: Optional[List[str]], top_k: int) -> List[tuple]:
        ...
        BM25检索（使用过滤后的文档）
        """
        # 由于LangChain的BM25Retriever.from_texts()对大规模数据不适用
        # 我们需要从数据库获取特定的文档进行BM25检索
        # 这里简化实现，返回空结果，实际应用中需实现完整BM25检索
        # 或使用PostgreSQL的内置全文搜索功能
        conn = self.vectorstore.connection
        cur = conn.cursor()
        ...
        try:
            ...
            if doc_ids:
                placeholders = ','.join(['%s'] * len(doc_ids))
                where_clause = f"WHERE dc.doc_id IN ({placeholders})"
                params.extend(doc_ids)
            
            # 使用PostgreSQL的全文搜索功能
            # 为简化，这里返回模拟结果
            # 实际实现需要集成PostgreSQL的全文搜索
            cur.execute(f"""
            ...
            # 对于BM25，我们可以使用PostgreSQL的tsvector功能
            # 这里简化实现，返回空结果
            return []  # 临时返回空，实际需要实现完整的BM25逻辑
...
class LangChainRAG:
    def __init__(self, llm, retriever, use_langsmith=True):
        self.llm = llm
