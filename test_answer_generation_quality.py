...
class TestAnswerGenerationExtended:
    ...
    @pytest.mark.asyncio
    async def test_mathematical_accuracy_compound_interest(self, answer_generator, mock_rag_engine):
        ...
        final_amounts = [float(n.replace('$', '').replace(',', '')) for n in numbers if float(n.replace('$', '').replace(',', '')) > 10000]

        expected = 11576.25
        if final_amounts:
            actual = final_amounts[-1]  # Get the final amount
            accuracy = 1 - abs(actual - expected) / expected
            assert accuracy > 0.98, f"Expected {expected}, got {actual}, accuracy: {accuracy}"
        else:
    ...
    async def test_fibonacci_accuracy(self, answer_generator, mock_rag_engine):
        ...
        if context_match:
            ...
            assert found_number == 55, f"Expected 55 as 10th Fibonacci number, got {found_number}"

    @pytest.mark.asyncio
    async def test_temperature_conversion_accuracy(self, answer_generator, mock_rag_engine):
        ...
        if celsius_values:
            ...
            accuracy = 1 - abs(actual - expected) / expected
            assert accuracy > 0.97, f"Expected {expected}°C, got {actual}°C, accuracy: {accuracy}"
        else:
    ...
    async def test_average_speed_calculation(self, answer_generator, mock_rag_engine):
        ...
        context = "Average speed = total distance / total time. Distance = speed × time."
        ...
        assert '53.33' in result['answer'] or '53.3' in result['answer'], "Expected to find correct average speed"
    ...
    async def test_discount_tax_calculation(self, answer_generator, mock_rag_engine):
        ...
        # Check for the correct final answer
        assert '86.40' in result['answer'] or '86.4' in result['answer'], "Expected to find correct final price"
    ...
    @pytest.mark.asyncio
    async def test_quantum_computing_explanation(self, answer_generator, mock_rag_engine):
        ...
        found_indicators = sum(1 for indicator in quality_indicators if indicator.lower() in result['answer'].lower())

        assert found_indicators >= 2, f"Expected at least 2 explanation quality indicators, found {found_indicators}"

        # Check for technical accuracy
        technical_terms = ['qubit', 'superposition', 'entanglement']
        found_technical = sum(1 for term in technical_terms if term.lower() in result['answer'].lower())
        assert found_technical >= 2, f"Expected at least 2 technical terms, found {found_technical}"
    ...
    async def test_mars_population_uncertainty(self, answer_generator, mock_rag_engine):
        ...
        context = "Mars currently has no permanent human population, only robotic missions"
        ...
        uncertainty_indicators = ['no permanent', 'zero', 'currently', 'as of now', 'does not have']
        found_indicators = sum(1 for indicator in uncertainty_indicators if indicator.lower() in result['answer'].lower())

        assert found_indicators >= 2, f"Expected at least 2 uncertainty indicators, found {found_indicators}"

        # Ensure it doesn't make up numbers
        numbers = re.findall(r'\b\d+\b', result['answer'])
        population_numbers = [int(n) for n in numbers if int(n) > 0 and int(n) < 1000000]
        assert len(population_numbers) == 0, "Should not include specific population numbers for Mars"

    @pytest.mark.asyncio
    async def test_flat_earth_misinformation_correction(self, answer_generator, mock_rag_engine):
        ...
        assert 'actually' in result['answer'].lower() or 'scientific evidence' in result['answer'].lower()

        # Check for evidence-based arguments
        evidence_indicators = ['satellite', 'gravity', 'circumnavigation', 'eclipse', 'horizon']
        found_evidence = sum(1 for indicator in evidence_indicators if indicator.lower() in result['answer'].lower())
        assert found_evidence >= 3, f"Expected at least 3 evidence types, found {found_evidence}"
    ...
    async def test_python_code_generation(self, answer_generator, mock_rag_engine):
        ...
        assert 'def bubble_sort' in result['answer'], "Expected bubble_sort function definition"

        # Check for explanation
        explanation_indicators = ['how it works', 'explanation', 'comment', 'time complexity']
        found_explanations = sum(1 for indicator in explanation_indicators if indicator.lower() in result['answer'].lower())
        assert found_explanations >= 2, f"Expected at least 2 explanation elements, found {found_explanations}"

    # ========== Cultural Sensitivity Tests ==========

    @pytest.mark.asyncio
    async def test_marriage_customs_cultural_sensitivity(self, answer_generator, mock_rag_engine):
        ...
        cultures_mentioned = ['western', 'indian', 'chinese', 'african', 'middle eastern']
        found_cultures = sum(1 for culture in cultures_mentioned if culture.lower() in result['answer'].lower())
        assert found_cultures >= 4, f"Expected at least 4 cultures mentioned, found {found_cultures}"

        # Check for neutral, respectful language
        respectful_indicators = ['diverse', 'tradition', 'cultural', 'heritage', 'reflect']
        found_respectful = sum(1 for indicator in respectful_indicators if indicator.lower() in result['answer'].lower())
        assert found_respectful >= 2, f"Expected respectful language indicators, found {found_respectful}"

        # Check for absence of judgmental language
        judgmental_words = ['strange', 'weird', 'wrong', 'inferior', 'primitive']
        found_judgmental = sum(1 for word in judgmental_words if word.lower() in result['answer'].lower())
        assert found_judgmental == 0, f"Found judgmental language: {found_judgmental} instances"

    # ========== Performance Tests ==========

    @pytest.mark.asyncio
    async def test_long_form_generation_quality(self, answer_generator, mock_rag_engine):
        """Test AG-ED-LG-001: Long-form generation quality"""
        query = "Comprehensive guide to getting started with machine learning"
        context = "Machine learning involves training algorithms to find patterns in data"
        ...
        heading_indicators = ['#', '##', '###']
        found_headings = sum(1 for line in result['answer'].split('\n')
                           for indicator in heading_indicators if line.strip().startswith(indicator))
        assert found_headings >= 5, f"Expected at least 5 headings for structure, found {found_headings}"

        # Check for comprehensive coverage
        key_topics = ['supervised', 'unsupervised', 'python', 'data', 'algorithm', 'learning']
        found_topics = sum(1 for topic in key_topics if topic.lower() in result['answer'].lower())
        assert found_topics >= 5, f"Expected at least 5 key topics covered, found {found_topics}"

        # Check coherence (connectors and flow)
        coherence_indicators = ['however', 'therefore', 'furthermore', 'additionally', 'finally', 'next']
        found_coherence = sum(1 for indicator in coherence_indicators if indicator.lower() in result['answer'].lower())
        assert found_coherence >= 2, f"Expected coherence indicators, found {found_coherence}"
    ...
    async def test_real_time_consistency(self, answer_generator, mock_rag_engine):
        ...
        assert confidence_variance < 0.01, f"Confidence scores too inconsistent: variance {confidence_variance}"
    ...
    def calculate_numerical_accuracy(self, response: str, expected: float, tolerance: float = 0.01) -> float:
    ...
    def assess_explanation_quality(self, response: str) -> Dict[str, float]:
        ...
        quality_metrics['clarity'] = sum(1 for word in clarity_words if word in response.lower()) / len(clarity_words)
        ...
        quality_metrics['fluency'] = max(0, 1 - (avg_sentence_length - 15) / 20)  # Optimal around 15 words/sentence
