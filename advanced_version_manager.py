...
class AdvancedVersionManager:
    ...
    def check_python_compatibility(self) -> Tuple[bool, str]:
        ...
        if current_py_version == target_version:
            return True, f"✅ Python版本完美匹配: {current_py_version[0]}.{current_py_version[1]} (目标版本)"
        else:
            return False, f"❌ Python版本不匹配: {current_py_version[0]}.{current_py_version[1]} ≠ {target_version[0]}.{target_version[1]} (目标版本)"

    def test_module_import(self, module_name: str, import_name: str = None) -> Tuple[bool, str]:
        """测试模块导入"""
        try:
    ...
    def check_test_dependencies(self, test_file_path: str) -> Dict:
        ...
        for module_name in critical_modules:
            if module_name in content.lower() or any(alias in content for alias in self._get_module_aliases(module_name)):
        ...
        # 判断是否可以运行
        critical_imported = all(m['imported'] for m in results['critical_modules'].values())
        ...
        for file_path in test_path.rglob("*.py"):
            if file_path.is_file():
                ...
                if file_path.suffix == '.py':
                    ...
                    results['analysis'][str(file_path.relative_to(test_path))] = analysis
                    ...
                    if analysis.get('can_run', False):
                        ...
                        missing_critical = sum(1 for m in analysis.get('critical_modules', {}).values()
                                              if not m.get('imported', False) and m.get('critical', False))
                        results['summary']['missing_critical_deps'] += missing_critical

                        missing_optional = sum(1 for m in analysis.get('optional_modules', {}).values()
                                              if not m.get('imported', False) and not m.get('critical', False))
        ...
        for file_path in resources_path.rglob("*"):
            if file_path.is_file():
                ...
                if file_ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tiff', '.gif']:
                    results['image_files'].append(str(file_path.relative_to(resources_path)))
                elif file_ext in ['.csv', '.json', '.xlsx', '.parquet']:
                    results['dataset_files'].append(str(file_path.relative_to(resources_path)))
                else:
                    results['other_files'].append(str(file_path.relative_to(resources_path)))
...
def main():
    ...
    if args.quiet:
        ...
        if args.save_report:
            try:
                filename = f"advanced_version_analysis_{time.strftime('%Y%m%d_%H%M%S')}.json"
