...
**Data Analysis page** (`/data-analysis`): Dark hero section (#1a1a2e) with system info (Cloud LLM · Docker Sandbox · 8 Chart Types). **SSE streaming pipeline**: "2) Run Analysis" triggers `POST /api/v1/data/analyze/start` → subscribes to `GET /api/v1/data/analyze/stream/{job_id}` for real-time progress. Pipeline visualization shows 3 stages (file_resolution → code_generation → visualization) with status dots (emerald=complete, amber=active, red=failed, gray=pending). Mobile: vertical layout, desktop: horizontal. Transparent fallback: when LLM code fails validation, pipeline shows amber warning + "Template Fallback" badge. Code generation mode badge: "LLM Generated" (green) or "Template Fallback" (amber) with fallback reason. "Include Visualization" toggle controls chart generation. 8 chart types (line, bar, scatter, histogram, box, heatmap, violin, pie). Backend result contract: `code_generation: { mode, fallback_reason }` (nested object, NOT flat `code_gen_mode`). Key Metrics whitelist: `success`, `analysis_type`, `execution_time`. Batch fallback (`handleRunAnalysisBatch`) available if SSE fails. E2E selectors use `data-testid` attributes (`run-analysis-btn`, `include-viz-toggle`, `analysis-pipeline-viz`, `code-gen-mode-badge`). Shared `useNodeAnimation` hook (`frontend/src/hooks/useNodeAnimation.ts`) powers pipeline animations across Dashboard, Intent Debugger, and Data Analysis.
