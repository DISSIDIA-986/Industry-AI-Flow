...
For user-uploaded datasets outside the pre-built cost model: extracts **metadata only** (not raw data — privacy by design) → sends metadata to cloud LLM (Groq/Zhipu, with dual fallback) for code generation → executes generated Python in Docker/E2B sandbox → returns results + visualizations. Cloud models used because local models are too weak for reliable code generation. Docker sandbox security-hardened (TDI rounds 28-36). E2B cloud sandbox also supported (`CODE_EXECUTION_PROVIDER=e2b`). **Visualization code gen**: LLM prompt must explicitly list `CodeValidator.BLOCKED_METHOD_NAMES` (`.apply()`, `.agg()`, `.map()`, `.pipe()`, `.query()`, `.eval()`) so generated code passes strict validation. `.transform()` is conditionally blocked — only rejected when given a Lambda argument (pandas groupby-callable footgun); `.transform("mean")`, `.transform(np.sqrt)`, `scaler.transform(X)`, and `pipeline.transform(X)` are all allowed. **CSV separator auto-sniff**: prompt instructs `pd.read_csv(path, sep=None, engine="python")` so semicolon/tab/pipe-delimited files (common in UCI-style datasets) don't produce `KeyError` at runtime. E2B provider downloads generated image files from `/workspace/` after execution.
...
**Data Analysis page** (`/data-analysis`): **Unified pipeline redesign** — 2-step flow (Upload+Preview → Ask+Analyze) with single merged LLM call for analysis+visualization. Dark hero section (#1a1a2e) with system info (Cloud LLM · Docker Sandbox · AI Charts). **Step 1**: file upload with data preview (column table: name/type/sample, desktop table + mobile card list). Default pre-loaded: `tips.csv`. `POST /api/v1/data/preview` endpoint returns metadata + first 5 rows. **Step 2**: natural language instruction input + amber "Run Analysis" button (`bg-amber-500`). AI auto-selects analysis type and chart type (no manual dropdowns). **Preset instruction chips** (5, Codex-curated, dataset-agnostic, ordered simple→advanced): EDA, segments/groupby, correlations/drivers, ML comparison, time-trends/anomalies. Click fills input (does not auto-run) so operator can tweak before Run. **Single-flight guard** on Run button: `inFlightRef` + `requestIdRef` token prevents double-click races and discards stale SSE events from orphaned jobs. **6-node SSE pipeline**: `file_parse → metadata_extract → code_generation → security_check → sandbox_execution → result_render` with SVG icons, emerald/amber/red status dots. Pipeline collapses to single-line summary after completion (click to expand). **Merged prompt**: `DataAnalysisAgent._build_code_generation_prompt()` generates analysis+visualization in one LLM call, blocked methods list dynamically from `CodeValidator.BLOCKED_METHOD_NAMES`. Output includes `ANALYSIS_SUMMARY_JSON` marker parsed via regex for structured key_findings. **Agentic CRISP-DM path success contract** (plan-eng-review 2026-04-20): envelope `success` follows `PlanExecutionResult.success` unconditionally; chart-persist failure becomes `code_generation.chart_persist_failed` telemetry (not a gate). `RoundRecord.is_successful()` soft-passes on `chart_exists OR summary_emitted` so LLM dropping the `ANALYSIS_SUMMARY_JSON` marker no longer fails a working chart. `_normalize_model_comparison_block()` passes real per-model numeric metrics through to the frontend metrics table (previously hardcoded `enabled=False` hid them). **Results**: Key Findings bullets (from `analysis_summary`), chart as primary visual (largest), **per-model metrics table when agentic returns model_comparison**, code gen mode badge (dark theme: `bg-emerald-900/20` / `bg-amber-900/20`), collapsible code/output/JSON sections. Previous analysis preserved in `<details>` section. `on_progress` callback in `analyze_query()` enables SSE without duplicating orchestration logic (DRY). E2E selectors: `data-testid` attributes (`run-analysis-btn`, `analysis-pipeline-viz`, `code-gen-mode-badge`, `data-preview`, `analysis-chart`, `analysis-results`, `instruction-presets`, `preset-chip-0..4`).
