...
For user-uploaded datasets outside the pre-built cost model: extracts **metadata only** (not raw data — privacy by design) → sends metadata to cloud LLM (Groq/Zhipu, with dual fallback) for code generation → executes generated Python in Docker/E2B sandbox → returns results + visualizations. Cloud models used because local models are too weak for reliable code generation. Docker sandbox security-hardened (TDI rounds 28-36). E2B cloud sandbox also supported (`CODE_EXECUTION_PROVIDER=e2b`). **Visualization code gen**: LLM prompt must explicitly list `CodeValidator.BLOCKED_METHOD_NAMES` (`.apply()`, `.agg()`, `.map()`, `.pipe()`, `.query()`, `.eval()`) so generated code passes strict validation. `.transform()` is conditionally blocked — only rejected when given a Lambda argument (pandas groupby-callable footgun); `.transform("mean")`, `.transform(np.sqrt)`, `scaler.transform(X)`, and `pipeline.transform(X)` are all allowed. **CSV separator auto-sniff**: metadata extraction (`_extract_dataset_info`) and the deterministic loaders probe candidate separators (`, ; \t |`) on the header (`nrows=0`), pick the first that yields >1 column, and fall back to plain comma — then to the lenient `sep=None, engine="python"` parser only on ragged rows. This keeps semicolon/tab/pipe-delimited UCI-style files parsing correctly **without** the single-column footgun where `csv.Sniffer` mis-detects a header letter as the delimiter (e.g. `value` → `['Unnamed: 0', 'alue']`). Mirrors `chart_executor._loader_block` so metadata and generated code agree on column shape. **Analysis tier planner** (`analysis_planner.py`): the agentic path gates advanced ML by a latency-aware tier — `skip` (EDA only) / `light` (one bounded model) / `full` (model comparison) — decided from dataset cost + explicit modeling intent. Target detection is **name-match only** (no last-column fallback), so EDA-only requests never auto-escalate to model training. Rendered as a hard prompt constraint via `tier_directive`; `decide_model_comparison` keeps its permissive last-column fallback for the separate feasibility path. E2B provider downloads generated image files from `/workspace/` after execution.
