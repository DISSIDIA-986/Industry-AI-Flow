...
def calculate_numerical_accuracy(response: str, expected: float, tolerance: float = 0.01) -> float:
    """计算数值答案的准确性"""
...
def assess_explanation_quality(response: str) -> Dict[str, float]:
    ...
    clarity_words = ['简单', '清楚', '理解', '解释', '步骤', '过程', 'simple', 'clear', 'understand', 'explain', 'concept']
    completeness_indicators = ['首先', '其次', '最后', '总结', '总之', 'first', 'second', 'finally', 'conclusion']
    structure_indicators = ['步骤', '第一步', '第二步', '第三步', '结论', 'steps', 'concepts', 'principles', 'approach']

    quality_metrics['clarity'] = min(len([w for w in clarity_words if w in response]) / len(clarity_words), 1.0)
    quality_metrics['completeness'] = min(len([w for w in completeness_indicators if w in response]) / len(completeness_indicators), 1.0)
    quality_metrics['structure'] = min(len([w for w in structure_indicators if w in response]) / len(structure_indicators), 1.0)
...
async def run_test_suite():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            result = await generator.generate_answer(test_case['query'], ["Mock context"])
            ...
            if test_case['category'] == 'Mathematical Accuracy':
                accuracy = calculate_numerical_accuracy(result['answer'], test_case['expected'], test_case['tolerance'])
                evaluation['numerical_accuracy'] = accuracy
                passed = accuracy >= test_case['target_accuracy']
                print(f"   数值准确度: {accuracy:.4f} (目标: {test_case['target_accuracy']}) {'✅' if passed else '❌'}")

            elif test_case['category'] == 'Multi-Step Reasoning':
                accuracy = calculate_numerical_accuracy(result['answer'], test_case['expected'], test_case['tolerance'])
                evaluation['reasoning_accuracy'] = accuracy
                passed = accuracy >= test_case['target_accuracy']
                print(f"   推理准确度: {accuracy:.4f} (目标: {test_case['target_accuracy']}) {'✅' if passed else '❌'}")
                ...
                print(f"   解释质量: {avg_quality:.4f} (目标: {test_case['target_quality']}) {'✅' if passed else '❌'}")

            elif test_case['category'] == 'Cultural Sensitivity':
                # 检查文化敏感性指标
                sensitive_terms = ['文化', '传统', '多样性', '尊重', '不同', 'cultural', 'traditions', 'diversity', 'different', 'societies', 'heritage']
                cultural_score = len([term for term in sensitive_terms if term in result['answer']]) / len(sensitive_terms)
                evaluation['cultural_sensitivity'] = cultural_score
                passed = cultural_score >= test_case['target_sensitivity']
                print(f"   文化敏感性: {cultural_score:.4f} (目标: {test_case['target_sensitivity']}) {'✅' if passed else '❌'}")

            elif test_case['category'] == 'Uncertainty Handling':
                # 检查不确定性表达
                uncertainty_terms = ['目前', '尚未', '没有', '0', '零', 'currently', 'no', 'zero', 'only', 'permanent']
                uncertainty_score = len([term for term in uncertainty_terms if term in result['answer']]) / len(uncertainty_terms)
                evaluation['uncertainty_handling'] = uncertainty_score
                passed = uncertainty_score >= test_case['target_uncertainty']
                print(f"   不确定性处理: {uncertainty_score:.4f} (目标: {test_case['target_uncertainty']}) {'✅' if passed else '❌'}")

            elif test_case['category'] == 'Misinformation Correction':
                # 检查错误信息纠正
                correction_terms = ['科学证据', '球形', '证明', '实际上', 'scientific', 'evidence', 'spherical', 'demonstrates', 'observations', 'satellite']
                correction_score = len([term for term in correction_terms if term in result['answer']]) / len(correction_terms)
                evaluation['misinformation_correction'] = correction_score
                passed = correction_score >= test_case['target_correction']
                print(f"   错误信息纠正: {correction_score:.4f} (目标: {test_case['target_correction']}) {'✅' if passed else '❌'}")
    ...
    try:
        ...
        report_file = output_dir / f"answer_generation_test_report_{time.strftime('%Y%m%d_%H%M%S')}.json"
