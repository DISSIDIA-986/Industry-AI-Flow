...
def test_architecture_datasets():
    ...
    try:
        ...
        print(f"❌ 建筑项目数据集错误: {e}")
        results.append(
            {"dataset": "building_projects", "status": "ERROR", "error": str(e)}
        )
    ...
    try:
        ...
        if materials_file.exists():
            ...
            print("❌ 建筑材料数据集不存在")
            results.append(
                {"dataset": "materials_properties", "status": "FILE_NOT_FOUND"}
            )

    except Exception as e:
        print(f"❌ 建筑材料数据集错误: {e}")
        results.append(
            {"dataset": "materials_properties", "status": "ERROR", "error": str(e)}
        )
...
def test_architecture_images():
    ...
    for img_file in images_path.glob("*.png"):
        if (
            "architect" in img_file.name.lower()
            or "construction" in img_file.name.lower()
        ):
    ...
    for img_file in architecture_images:
        ...
        results.append(
            {"image": img_file.name, "size_kb": size_kb, "status": "SUCCESS"}
        )
...
def test_architecture_queries():
    ...
    for i, query_info in enumerate(queries, 1):
        ...
        found_keywords = [
            kw for kw in query_info["keywords"] if kw.lower() in query_lower
        ]
...
def generate_summary_report(
    ...
    print(
        f"   数据分析: {analysis_success}/{analysis_total} ({analysis_success/analysis_total*100:.1f}%)"
    )
    print()
    ...
    if "building_projects" in [r.get("dataset") for r in dataset_results]:
        building_result = next(
            r for r in dataset_results if r.get("dataset") == "building_projects"
        )
    ...
    if "materials_properties" in [r.get("dataset") for r in dataset_results]:
        materials_result = next(
            r for r in dataset_results if r.get("dataset") == "materials_properties"
        )
    ...
    try:
        report_data = {
            ...
            "summary": {
                ...
                "analysis_success_rate": analysis_success / analysis_total
                if analysis_total > 0
                else 0,
        ...
        report_file = (
            output_dir
            / f"architecture_test_summary_{time.strftime('%Y%m%d_%H%M%S')}.json"
        )
